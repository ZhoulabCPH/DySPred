...
from embedding.baseline.gcn import GCN, TgGCN
...
class GCRN(nn.Module):
    ...
    def __init__(self, input_dim, feature_dim, hidden_dim, output_dim, feature_pre=True, layer_num=2, dropout=0., bias=True, duration=1, rnn_type='GRU'):
    ...
    def forward(self, x_list, edge_list):
        ...
        for i in range(time_num):
            x = self.gcn_list[i](x_list[i], edge_list[i][0])
            x = F.normalize(x, p=2)
            hx_list.append(x)
